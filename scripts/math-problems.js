...
function long_addition(input1, input2){
	...
	for (var step = 0; step < stepList.length; step++){
		...
		description_div.innerHTML = "<b>"+"Step " + (step+1) + "." + "</b>";
		problem_div.appendChild(description_div);
		
		var description_div = document.createElement('div');
		description_div.innerHTML = stepList[step]['description'];
		problem_div.appendChild(description_div);
		
		var latex_div = document.createElement('div');
		latex_div.innerHTML = '$$' + stepList[step]['latex'] + '$$';
		problem_div.appendChild(latex_div);
